...
export default async function AnalyticsPage() {
  ...
  return (
    <main className="mx-auto max-w-md space-y-4 px-4 py-8">
      ...
      <p className="text-xs text-neutral-600">Últimos {a.rangeDays} días</p>

      <section className="grid grid-cols-2 gap-4">
        <Card>
          <p className="text-xs uppercase text-neutral-600">Verificación</p>
          <p className="mt-1 text-2xl font-bold">{Math.round(a.verificationRate * 100)}%</p>
          <p className="text-xs text-neutral-600">{a.verified} verif · {a.skipped} saltados</p>
        </Card>
        <Card>
          <p className="text-xs uppercase text-neutral-600">Tiempo verificado</p>
          <p className="mt-1 text-2xl font-bold">{Math.round(a.plannedVerifiedMinutes / 60)}h</p>
          <p className="text-xs text-neutral-600">planificado y verificado</p>
        </Card>
        <Card>
          <p className="text-xs uppercase text-neutral-600">Consumo IA</p>
          <p className="mt-1 text-2xl font-bold">${a.creditsSpentUsd.toFixed(2)}</p>
        </Card>
        <Card>
          <p className="text-xs uppercase text-neutral-600">Bloques</p>
          <p className="mt-1 text-2xl font-bold">{a.blocksTotal}</p>
        </Card>
      </section>

      <Card>
        <p className="mb-2 text-xs uppercase text-neutral-600">Patrón de energía (verificaciones por hora UTC)</p>
