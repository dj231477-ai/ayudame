...
>
> **Versión:** 2.1.18 · **Fecha:** Agosto 2026 · **Estado:** en producción.
>
...
>
> **Cambios en 2.1.18 (agosto 2026).** Nueva herramienta opt-in en §C-18.6: `npm run inspect`, un inspector de página con navegador real (`apps/flowday/scripts/inspect-page.mjs`) que reusa el Chromium que ya instala Playwright, sin dependencias nuevas. Vuelca consola, errores de JS, peticiones fallidas, respuestas 4xx/5xx, cabeceras de §C-8.7 y desbordamiento horizontal a 375px (INV-10). Cubre el hueco entre Vitest (no ve el navegador) y Playwright E2E (verifica un flujo concreto, no diagnostica una página suelta) — justo la clase de fallo silencioso de D-18/D-19/D-22. Aviso operativo: al regenerar el `package-lock.json` (2.1.17) Playwright subió de versión y sus navegadores instalados dejaron de coincidir; hace falta `npm run test:e2e:install` (o `npx playwright install chromium`) una vez, o tanto `test:e2e` como `inspect` fallan con "Executable doesn't exist".
...
| Inspector de página | `npm run inspect -- <url> [--viewport WxH] [--shot f.png]` | Abre la URL en el Chromium de Playwright y vuelca lo que ningún test unitario ve: mensajes y errores de consola, errores de JS no capturados, peticiones fallidas, respuestas 4xx/5xx, presencia de las cabeceras de §C-8.7 y desbordamiento horizontal (INV-10). Por defecto 375×812. Sale con código 1 si hay errores de consola o peticiones fallidas, así que sirve como comprobación puntual además de para diagnosticar. |
